...
`hoomd-heat transfer` provides a framework to ... that extend
...
2. Obtain the component's source.
    ```
    $ git clone https://github.com/glotzerlab/hoomd-heat-transfer
    ```
3. Configure.
    ```
    $ cmake -B build/hoomd-heat-transfer -S hoomd-heat-transfer
    ```
    or 
    ```
    $ cmake -B build/hoomd-heat-transfer -S hoomd-heat-tranfer -DPython_EXECUTABLE=/path/to/virtual/env/bin/python3
    ```
